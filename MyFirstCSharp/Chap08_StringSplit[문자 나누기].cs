...
namespace MyFirstCSharp
{
    public partial class Chap08_StringSplit : Form
    {
        ...
        {
            // Split : 지정한 문자를 기준으로 문자열(열거형데이터)를 자르고 자른 값의 결과를 배열 형태로 반환한다.
            ...
            MessageBox.Show($"타이틀의 / 기준 문단 개수는 {sValues.Length} 개입니다.");

            // 배열의 첫번째 문단 ? 
            MessageBox.Show($"타이틀의 / 기준 첫 문단은 {sValues[0]}입니다.");

            // 배열의 마지막 문단 ?
            MessageBox.Show($"타이틀의 / 기준 마지막 문단은 {sValues[sValues.Length -1]}");
        ...
        private void btnSubstring_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show($"타이틀 {sTitle}의 {iIndex}의 주소로부터 {iCount}개의 문자를 자른 결과는 {sValue}입니다.");

            // 위의 로직을 한 행으로 표현할 때는 아래처럼 구현할 수 있다. (In Line 코드)
            MessageBox.Show($"타이틀 {lblTitle.Text}의 {txtIndex.Text}째 주소로부터 {txtCount.Text}개의 문자를 자른 결과는 {lblTitle.Text.Substring(Convert.ToInt32(txtIndex.Text), Convert.ToInt32(txtCount.Text))}입니다.");
