...
namespace MyFirstCSharp
{
    public partial class Chap15_Array : Form
    {
        ...
        // - C# 에서 배열은 반드시 방의 개수를 지정하여 생성하거나 배열에 나열될 값들의 개수가 명시되어야 한다. (Ex. new int[5] , Split('/') )
        ...
        private void btnArrayF_Click(object sender, EventArgs e)
        {
            ...
            int[] iValues4 = { 10, 20, 30, 40, 50 };

            // 반환형태가 배열인 기능(메서드)을 이용하여 배열을 등록
            string[] sValues = "ABCDE-FGHI".Split('-');

            // 기존에 생성되어있는 배열의 방 갯수만큼 빈 배열 만들기
            ...
            // 깊은 복사 방법
            // - 참조 형식의 데이터를 복사할 때 주소와 값의 정보를 그대로 복사하여 서로 다른 데이터 형식으로 복사하는 방법

            object oValue = iExamArray.Clone();                
            int[] iBackUps = (int[])oValue; // Object 형식 -> 특정데이터 형식 형변환 unBoxing
        ...
        private void btnArrayCopy_Click(object sender, EventArgs e)
        {
            ...
            // 얕은 복사
            // 참조 형식의 데이터를 복사할 때 값이 저장된 위치 정보만 절달해주는 복사방법
