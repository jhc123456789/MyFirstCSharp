...
{
    public partial class lblTitle2 : Form
    {
        // Foreach
        // - 열거형 데이터 자료구조에서 데이터를 추출할 때 사용
        //   열거형 자료구조 : 일정한 데이터 자료가 동일한 경우 하나의 열거형 데이터로 집합할 수 있는 자료구조
        //                    배열, 문자열, 리스트...
        // - 조건의 끝을 지정해주는 반복문 (for, while) 과는 달리 자료형 구조가 포함하고 있는 데이터의 수에 따라 반복하므로 반드시 종료가 되는 반복문.
        ...
        private void btnStringForeach_Click(object sender, EventArgs e)
        {
            ...
            // txtInputWord.Text가 문자열 (문자의 배열) length를 사용할 수 있다.
            ...
            if (bFindFlag)
            {
                sMessage = $"{cValue} 문자는 포함되어 있습니다.";

            }
            else
            {
                sMessage = "문자가 포함되어있지 않습니다.";
            }
        ...
        private void btnArrayForeach_Click(object sender, EventArgs e)
        {
            ...
            // foreach가 반복하는 횟수(배열의 개수)
            int iCount = 0;


            foreach (int iValue in iValues)
            {
                MessageBox.Show($"배열의 {iCount}번째 주소에 있는 값은 {iValue} 입니다.");
                iCount++;
            }
        ...
        {
            // 컨테이너 안에 있는 텍스트 박스 일괄 초기화하기

            // 아래의 로직은 확장성과 유지보수성이 떨어지며 반복적으로 표현하는 구문이 존재하므로 비효율적인 코딩이 될 수 있다.
            //textBox1.Text = "";
            //textBox2.Text = "";
            //textBox3.Text = "";
            //textBox4.Text = "";
            ...
            // 그룹박스에 포함되어 있는 컨트롤(도구)를 하나씩 추출하여 텍스트 박스일 경우 텍스트 속성의 내용을 지우는 로직을 구현
            // Winform 에서는 도구의 클래스를 (Control, Component) 라고 부른다.

            // 그룹박스의 컨트롤 들을 하나씩 추출하여 컨트롤 클래스 MyControl로 받아낸다.
            foreach (Control MyControl in grpControls.Controls)
            {
                // is : 해당 클래스인지, 해당 클래스를 상속받았는지에 대한 여부 판단 후 결과를 반환
                // 만약에 MyControl이 TextBox 유형이면
